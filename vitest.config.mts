...
export default defineConfig({
  resolve: {
    alias: {
      ...
      // point of it — but that makes every server module untestable. Next's bundler
      // still resolves the real package during `npm run build`, so the guarantee that
      // src/server never reaches the browser bundle is unaffected.
