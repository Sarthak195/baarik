...
| 4 | **Answer questions from the document** | `src/server/pipeline/stages.ts` (`answerQuestion`), `src/schemas/qa-answer.ts` | Strictly grounded. `foundInDocument: false` is a first-class answer with its own schema field: a reader who asks about provident fund and gets a plausible invented figure has been actively misled. *(Pipeline stage; no HTTP route in this build — see `docs/LIMITATIONS.md`.)* |
