...
| **Efficiency** | Medium | The document is folded **once per verification pass**, not once per quote: O(document + quotes × document). Cost is flat in quote width — the fuzzy scan visits ~8L/w coarse positions at O(w) each, so width cancels. Measured: 16× the quote width costs 1.8× the time; 16× the document costs 20×. | [`src/core/grounding/verify.ts`](src/core/grounding/verify.ts) |
