...
| **Cost / quota exhaustion** | A scripted loop burning the free-tier Gemini quota so the demo is dead for everyone else | Quota is the scarce resource, and it is protected structurally: the sample analyses are precomputed in `src/lib/demo/` and cost **zero** API calls, so the highest-traffic path never touches the model. A 303-redirect POST means a refresh cannot re-post the document. Classification runs on the cheapest model over the first 4,000 characters only. A rotating key pool demotes a rate-limited key and a model ladder substitutes a sibling model rather than failing. `LIMITS.rateLimit` (capacity 12, refill 4/min) is declared — see *Known gaps* below | `src/lib/demo/`, `src/app/analyze/route.ts`, `src/server/config/key-pool.ts`, `src/server/genai/gateway.ts`, `src/server/config/limits.ts` |
